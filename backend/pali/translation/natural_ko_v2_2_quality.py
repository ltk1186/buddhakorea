...
def glossary_lock_entries_v2_2() -> list[GlossaryEntry]:
    return [
        ...
        GlossaryEntry("yama", ["금계"], [], [], "Needs project/expert confirmation before full glossary promotion.", lock_status="needs_expert_confirm"),
        GlossaryEntry("niyama", ["권계"], [], [], "Needs project/expert confirmation before full glossary promotion.", lock_status="needs_expert_confirm"),
        ...
        GlossaryEntry("khārena paripphositvā", ["잿물을 뿌리고서"], [], ["상처에 잿물을 뿌리고서"], "Do not add 상처 unless source/context says wound."),
        ...
        GlossaryEntry("nanabhāvanāya / nasaraṇaṃ-type double negatives", ["원인/닦음/귀의 관련 부정 범위를 보존"], [], ["번뇌를 동반하지 않으며"], "Paṭṭhāna/double-negation risk; mark uncertainty if unsure."),
        GlossaryEntry("appavatti", ["맥락에 따라 비전전/일어나지 않음/작용하지 않음"], [], [], "Context-sensitive; do not force obscure calque.", lock_status="needs_expert_confirm"),
        GlossaryEntry("otaraṇā", ["끌어들여 해석함", "적용하여 해석함"], ["들어감"], ["맥락 없는 들어감"], "Context-sensitive commentarial function.", lock_status="needs_expert_confirm"),
        ...
        GlossaryEntry("viññatti", ["암시", "표의", "의사표시"], [], [], "Needs context and project convention confirmation.", lock_status="needs_expert_confirm"),
        GlossaryEntry("virati", ["절제", "삼감"], [], [], "Stable ethical term."),
        GlossaryEntry("appanā", ["본삼매"], [], [], "Project convention term."),
        GlossaryEntry("kalyāṇamitta", ["훌륭한 친구", "선지식"], [], [], "Project convention/context dependent.", lock_status="needs_expert_confirm"),
        GlossaryEntry("ājīvika", ["생활수단", "생계", "아지비카"], [], [], "Sect vs livelihood is an expert crux.", lock_status="needs_expert_confirm"),
...
def detect_known_omissions(item: dict[str, Any]) -> list[str]:
    ...
    for fragment, required_phrases in KNOWN_OMISSION_CHECKS.items():
        if has_key_fragment(key, fragment) and not any(phrase in text for phrase in required_phrases):
            omissions.append("missing_accenti_jinaganda")
    return omissions
