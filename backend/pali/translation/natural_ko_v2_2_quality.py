...
class GlossaryEntry:
    ...
    disallowed_enforcement: Literal["hard", "aligned_hard", "advisory"] = "hard"
...
def glossary_lock_entries_v2_2() -> list[GlossaryEntry]:
    return [
        ...
        GlossaryEntry("khandha", ["무더기"], ["오온"], ["무리"], "Default is 무더기; natural_ko may support first occurrence with 오온 where helpful. Disallowed 무리 is aligned_hard: hard only when terms[] maps khandha to 무리; otherwise advisory.", disallowed_enforcement="aligned_hard", trigger_patterns=("khandha", "khandh", "kkhandh")),
        ...
        GlossaryEntry("yama", ["금계"], [], ["맥락 없는 금계"], "Needs project/expert confirmation before full glossary promotion.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("yama", "yam")),
        GlossaryEntry("niyama", ["권계"], [], ["맥락 없는 권계"], "Needs project/expert confirmation before full glossary promotion.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("niyama", "niyam")),
        ...
        GlossaryEntry("appavatti", ["맥락에 따라 비전전/일어나지 않음/작용하지 않음"], [], ["맥락 없는 비전전"], "Context-sensitive; do not force obscure calque.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("appavatti", "appavatt")),
        GlossaryEntry("otaraṇā", ["끌어들여 해석함", "적용하여 해석함"], ["들어감"], ["맥락 없는 들어감"], "Context-sensitive commentarial function.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("otaraṇ", "otaraṇā")),
        ...
        GlossaryEntry("viññatti", ["암시", "표의", "의사표시"], [], ["맥락 없는 표의"], "Needs context and project convention confirmation.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("viññatti", "viññatt")),
        ...
        GlossaryEntry("kalyāṇamitta", ["훌륭한 친구", "선지식"], [], ["맥락 없는 선지식"], "Project convention/context dependent.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("kalyāṇamitta", "kalyāṇamitt")),
        GlossaryEntry("ājīvika", ["생활수단", "생계", "아지비카"], [], ["맥락 없는 아지비카"], "Sect vs livelihood is an expert crux.", lock_status="needs_expert_confirm", enforcement_level="advisory", disallowed_enforcement="advisory", confidence="needs_expert_confirm", trigger_patterns=("ājīvika", "ājīvik", "ājīvaka", "ājīvak")),
        ...
        "entries": [
            {
                ...
                "disallowed_enforcement": entry.disallowed_enforcement,
...
def translation_payload(item: dict[str, Any]) -> dict[str, Any]:
    value = item.get("parsed_translation_json") or item.get("translation") or item
...
def source_or_terms_text(item: dict[str, Any]) -> str:
    ...
    translation = translation_payload(item)
    for term in translation.get("terms") or item.get("terms") or []:
...
def term_matches_entry(entry: GlossaryEntry, pali: str) -> bool:
    normalized = pali.casefold()
    patterns = entry.trigger_patterns or (entry.pali.casefold(),)
    return any(pattern.casefold() in normalized for pattern in patterns if pattern)


def terms_align_disallowed_rendering(entry: GlossaryEntry, item: dict[str, Any], phrase: str) -> bool:
    translation = translation_payload(item)
    for term in translation.get("terms") or item.get("terms") or []:
        ...
        if term_matches_entry(entry, pali) and phrase in ko:
            return True
    return False
...
def classify_bracket_spans(item: dict[str, Any]) -> list[dict[str, Any]]:
    text = output_text(item)
    source = str(item.get("original_text") or item.get("source_text") or "")
    findings: list[dict[str, Any]] = []
    for match in BRACKET_SPAN_RE.finditer(text):
        bracket_text = match.group(0)
        content = bracket_text[1:-1].strip()
        in_source = bracket_text in source or (bool(content) and content in source)
        if in_source:
            bracket_type = "SOURCE_FAITHFUL"
            gating = False
        elif HANGUL_RE.search(content):
            bracket_type = "SUPPLIED_KOREAN"
            gating = True
        elif content.isdigit():
            bracket_type = "SUPPLIED_NUMBER"
            gating = False
        elif CJK_RE.match(content):
            bracket_type = "HANJA_GLOSS"
            gating = False
        else:
            bracket_type = "OTHER_BRACKET"
            gating = False
        findings.append(
            {
...
def detect_bracket_violations(item: dict[str, Any]) -> list[dict[str, Any]]:
    return [finding for finding in classify_bracket_spans(item) if finding["gating"]]


def detect_bracket_advisories(item: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        finding
        for finding in classify_bracket_spans(item)
        if not finding["gating"] and finding["bracket_type"] != "SOURCE_FAITHFUL"
    ]


def detect_source_faithful_brackets(item: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        finding
        for finding in classify_bracket_spans(item)
        if finding["bracket_type"] == "SOURCE_FAITHFUL"
    ]
...
def detect_glossary_violations(item: dict[str, Any]) -> list[str]:
    ...
    violations: list[str] = []
    for entry in glossary_lock_entries_v2_2():
        if entry.enforcement_level != "hard" or entry.disallowed_enforcement == "advisory":
            continue
        ...
        for phrase in entry.disallowed_renderings:
            if not phrase:
                continue
            if entry.disallowed_enforcement == "aligned_hard":
                if terms_align_disallowed_rendering(entry, item, phrase):
                    violations.append(f"{entry.pali}:{phrase}")
                continue
            if phrase in text or terms_align_disallowed_rendering(entry, item, phrase):
                violations.append(f"{entry.pali}:{phrase}")
    return sorted(set(violations))
...
def detect_advisory_glossary_warnings(item: dict[str, Any]) -> list[str]:
    ...
    for entry in glossary_lock_entries_v2_2():
        ...
        for phrase in entry.disallowed_renderings:
            if not phrase:
                continue
            if entry.disallowed_enforcement == "hard":
                continue
            hard_aligned = entry.disallowed_enforcement == "aligned_hard" and terms_align_disallowed_rendering(entry, item, phrase)
            if hard_aligned:
                continue
            if phrase and (phrase in text or terms_align_disallowed_rendering(entry, item, phrase)):
...
def evaluate_d_arm_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    bracket_advisory = detect_bracket_advisories(item)
    source_faithful_brackets = detect_source_faithful_brackets(item)
    ...
    return {
        ...
        "bracket_advisory_warning": bool(bracket_advisory),
        "bracket_advisory_warning_details": bracket_advisory,
        "source_faithful_brackets": source_faithful_brackets,
...
def summarize_d_arm_gates(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "bracket_advisory_warnings": sum(check["bracket_advisory_warning"] for check in checks),
        "source_faithful_brackets": sum(len(check["source_faithful_brackets"]) for check in checks),
