...
class GoldenSetBuilder:
    ...
    def generate_questions_for_sutra(
        ...
        prompt = f"""당신은 불교 교육 평가 전문가입니다. 다음 경전에 대한 평가용 질문-답변 쌍을 생성하세요.
...
1. **factual** (사실 질문): 경전의 구체적 내용에 대한 질문
   - 예: "이 경전에서 설명하는 사성제는 무엇인가요?"

2. **interpretive** (해석 질문): 가르침의 의미를 해석하는 질문
   - 예: "이 경전에서 공(空) 사상은 어떤 의미를 가지나요?"

3. **practical** (실천 질문): 수행과 관련된 질문
   - 예: "이 경전에 따르면 어떻게 명상을 실천해야 하나요?"

**질문 작성 지침:**
- 자연스러운 한국어로 작성
- 초보자도 이해할 수 있는 명확한 언어 사용
- 경전의 구체적 내용을 참조할 수 있는 질문
- 너무 일반적이거나 모호하지 않게

**답변 작성 지침:**
- 경전의 내용을 바탕으로 정확하게 작성
- 200-400자 분량
- 핵심 개념을 명확히 설명
- 필요시 예시 포함
