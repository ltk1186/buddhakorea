...
def generate_korean_summary(
    ...
    prompt = f"""당신은 불교 경전 전문가입니다. 다음 CBETA 대장경 텍스트의 한글 번역 및 요약을 작성해주세요.
...
2. **간략 요약** (brief_summary): 이 경전의 핵심 내용을 100-150자로 요약. 사용자가 목록에서 이 경전이 무엇인지 즉시 이해할 수 있도록.
...
원문 샘플이 충분하지 않거나 경전 ID/제목만으로 알 수 있는 유명한 경전의 경우, 불교학 지식을 바탕으로 작성하세요.
"""
